...
import {
  ...
  Heart, Languages, BarChart2, ListChecks, QrCode, LayoutTemplate, X, KeyRound, ThumbsUp, ThumbsDown
} from 'lucide-react';
import {
...
// --- DEMO RESUME DATA FOR INSTANT EXPERIENCE ---
const DEMO_RESUME_DATA = {
  personalInfo: {
    fullName: 'Alex Carter',
    email: 'alex.carter@tech.com',
    phone: '+1 (415) 555-0192',
    location: 'San Francisco, CA',
    website: 'alexcarter.dev',
    github: 'https://github.com/alexcarter',
    linkedin: 'https://linkedin.com/in/alexcarter',
    summary: 'Senior Software Engineer with 6+ years of experience specializing in building highly scalable web systems, distributed APIs, and cloud-native application architectures. Expert in JavaScript, React, Node.js, and Docker.',
  ...
  education: [
    {
      ...
      endDate: '2020-06',
      grade: '3.85 GPA',
      description: 'Specialization in Software Systems and Distributed Databases.'
    },
    {
      id: 'edu-2',
      ...
      endDate: '2018-06',
      grade: '3.9 GPA',
      description: 'Graduated with Honors. Regatta Software Club lead.'
    }
  ],
  experience: [
    {
      ...
      endDate: 'Present',
      current: true,
      description: 'Architected and deployed scalable backend microservices using Node.js, GraphQL, and AWS RDS PostgreSQL, reducing API response times by 32%.\nLed a team of 4 engineers to migrate legacy monoliths to Docker and Kubernetes orchestration.\nSpearheaded CI/CD automation pipelines using GitHub Actions, boosting sprint delivery speed by 20%.'
    },
    {
      id: 'exp-2',
      ...
      endDate: '2021-12',
      current: false,
      description: 'Developed responsive, high-performance web interfaces in React and Tailwind CSS, increasing page engagement by 45%.\nImplemented global state caching layers using Redis and client-side storage, resulting in optimized bandwidth utility.\nCollaborated directly with product design teams in Figma to build a modular UI design system.'
    }
  ],
  projects: [
    {
      id: 'proj-1',
      title: 'Scalable E-Commerce Storefront',
      role: 'Backend Architect',
      technologies: 'Node.js, Redis, PostgreSQL, AWS S3',
      link: 'github.com/alexcarter/ecommerce-storefront',
      description: 'Designed a low-latency checkout engine with optimistic locking to process up to 5,000 concurrent orders without data collision.'
    },
    {
      id: 'proj-2',
      title: 'NLP Document Classifier',
      role: 'Machine Learning Developer',
      technologies: 'Python, Scikit-Learn, Flask, TF-IDF',
      link: 'github.com/alexcarter/nlp-classifier',
      description: 'Created a text categorization pipeline with 94% precision to automate invoice and metadata tagging for legal documents.'
    }
  ],
  skills: {
    technical: ['JAVASCRIPT', 'EXPRESS', 'REACT', 'NODE.JS', 'PYTHON', 'DOCKER', 'KUBERNETES', 'AWS', 'SQL', 'REDIS', 'GIT', 'FIGMA', 'NEXT.JS', 'CI/CD'],
    soft: ['Team Leadership', 'Problem Solving', 'System Architecture', 'Agile Methodologies', 'Effective Communication']
  },
  certifications: [
    {
      id: 'cert-1',
  ...
  languages: [
    { id: 'lang-1', name: 'English', proficiency: 'Native Proficiency' },
    { id: 'lang-2', name: 'Spanish', proficiency: 'Conversational' }
  ],
  interests: ['Distributed Systems', 'Generative AI Art', 'Backpacking', 'Sailing']
};
...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'auth' | 'dashboard' | 'builder' | 'analyzer' | 'history' | 'admin'>(() => {
    try {
      return localStorage.getItem('resumatch_token') ? 'dashboard' : 'auth';
    } catch {
      return 'auth';
    }
  });
  ...
  const fetchDashboardData = async (activeToken: string) => {
    try {
      ...
      if (Array.isArray(data)) {
        setResumes(data);
        // Auto-load demo resume if none exist
        if (data.length === 0) {
          handleCreateResumeWithData('My Technical Resume', DEMO_RESUME_DATA, 'template-software', activeToken);
        }
      } else {
  ...
  const handleCreateResumeWithData = async (title: string, dataObj: typeof DEMO_RESUME_DATA, selectedTemplateId = 'template-software', activeToken = token) => {
    try {
  ...
  const triggerParsing = (file: File) => {
    ...
    reader.onload = async () => {
      try {
        ...
        handleCreateResumeWithData(
          `Imported - ${file.name.split('.')[0]}`,
          data.parsedData
        );
        showToast('Document parsed with advanced NLP matching!', 'success');
      } catch (err) {
        setIsParsingFile(false);
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(data.error || 'Failed to complete analysis.', 'error');
      ...
      if (suggestRes.ok) {
        ...
        setAiSuggestions({
          ...
          scoreImpact: {
            currentScore: data.analysis.atsScore,
            estimatedScore: Math.min(100, data.analysis.atsScore + 10),
            potentialIncrease: 10
          },
          positiveFeedback: 'Review your general formatting for compliance and add advanced technical certificates.'
        });
      }
      setIsAnalyzing(false);
    } catch (err) {
      setIsAnalyzing(false);
      setIsRequestingSuggestions(false);
      showToast('Network error while running Machine Learning pipeline.', 'error');
    }
  };
  ...
  const getAtsRadarData = () => {
    if (!analysisResult || !analysisResult.scoringBreakdown) {
      return [
        ...
        { subject: 'Education', score: 85 },
        { subject: 'ATS Formatting', score: 90 }
      ];
    }
  ...
  return (
    <div className={`min-h-screen flex flex-col bg-slate-50 text-slate-900 ${activeResume ? FONTS.find(f => f.id === activeResume.customization.fontFamily)?.css || 'font-sans' : 'font-sans'}`} id="app-container">
      <input
        type="file"
        ref={fileInputRef}
            ...
            <div className="lg:col-span-5 space-y-6" id="builder-forms-column">
              ...
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-2 border border-slate-100 hover:bg-slate-50 text-slate-600 rounded-xl transition"
                ...
                <div className="space-y-3 p-4 bg-slate-50 rounded-2xl">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          updated.data.personalInfo.fullName = e.target.value;
                          setActiveResume(updated);
                          handleUpdateResume(updated);
                        }}
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const updated = { ...activeResume };
                          updated.data.personalInfo.title = e.target.value;
                          setActiveResume(updated);
                          handleUpdateResume(updated);
                        }}
                        className="w-full text-xs p-2 bg-white border rounded-lg focus:ring-1 focus:ring-indigo-500 outline-none"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const updated = { ...activeResume };
                          updated.data.personalInfo.email = e.target.value;
                          setActiveResume(updated);
                          handleUpdateResume(updated);
                        }}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const updated = { ...activeResume };
                          updated.data.personalInfo.phone = e.target.value;
                          setActiveResume(updated);
                          handleUpdateResume(updated);
                        }}
                        className="w-full text-xs p-2 bg-white border rounded-lg"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const updated = { ...activeResume };
                          updated.data.personalInfo.location = e.target.value;
                          setActiveResume(updated);
                          handleUpdateResume(updated);
                        }}
                        className="w-full text-xs p-2 bg-white border rounded-lg"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const updated = { ...activeResume };
                          updated.data.personalInfo.website = e.target.value;
                          setActiveResume(updated);
                          handleUpdateResume(updated);
                        }}
                        className="w-full text-xs p-2 bg-white border rounded-lg"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const updated = { ...activeResume };
                          updated.data.personalInfo.github = e.target.value;
                          setActiveResume(updated);
                          handleUpdateResume(updated);
                        }}
                        className="w-full text-xs p-2 bg-white border rounded-lg"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const updated = { ...activeResume };
                          updated.data.personalInfo.linkedin = e.target.value;
                          setActiveResume(updated);
                          handleUpdateResume(updated);
                        }}
                        className="w-full text-xs p-2 bg-white border rounded-lg"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => {
                        const updated = { ...activeResume };
                        updated.data.personalInfo.summary = e.target.value;
                        setActiveResume(updated);
                        handleUpdateResume(updated);
                      }}
                      ...
                      <div key={exp.id} className="p-3 bg-white rounded-xl border border-slate-100 space-y-2 relative">
                        ...
                        <div className="grid grid-cols-2 gap-2 pr-20">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = { ...activeResume };
                                updated.data.experience[index].company = e.target.value;
                                setActiveResume(updated);
                                handleUpdateResume(updated);
                              }}
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = { ...activeResume };
                                updated.data.experience[index].position = e.target.value;
                                setActiveResume(updated);
                                handleUpdateResume(updated);
                              }}
                              className="w-full text-xs p-1.5 border rounded"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const updated = { ...activeResume };
                                updated.data.experience[index].startDate = start.trim();
                                updated.data.experience[index].endDate = end;
                                setActiveResume(updated);
                                handleUpdateResume(updated);
                              }}
                              className="w-full text-xs p-1.5 border rounded"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = { ...activeResume };
                                updated.data.experience[index].location = e.target.value;
                                setActiveResume(updated);
                                handleUpdateResume(updated);
                              }}
                              className="w-full text-xs p-1.5 border rounded"
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) => {
                              const updated = { ...activeResume };
                              updated.data.experience[index].description = e.target.value;
                              setActiveResume(updated);
                              handleUpdateResume(updated);
                            }}
                        ...
                        <div className="grid grid-cols-2 gap-2 pr-10">
                          <div>
                            <input
                              ...
                              onChange={(e) => {
                                const updated = { ...activeResume };
                                updated.data.education[index].institution = e.target.value;
                                setActiveResume(updated);
                                handleUpdateResume(updated);
                              }}
                              className="w-full text-xs p-1.5 border rounded"
                          ...
                          <div>
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const updated = { ...activeResume };
                                updated.data.education[index].degree = degree.trim();
                                updated.data.education[index].fieldOfStudy = fieldOfStudy;
                                setActiveResume(updated);
                                handleUpdateResume(updated);
                              }}
                              className="w-full text-xs p-1.5 border rounded"
                      ...
                      <div key={proj.id} className="p-3 bg-white rounded-xl border border-slate-100 space-y-2 relative">
                        ...
                        <div className="space-y-2 pr-10">
                          <input
                            ...
                            onChange={(e) => {
                              const updated = { ...activeResume };
                              updated.data.projects[index].title = e.target.value;
                              setActiveResume(updated);
                              handleUpdateResume(updated);
                            }}
                            className="w-full text-xs p-1.5 border rounded"
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const updated = { ...activeResume };
                              updated.data.projects[index].technologies = e.target.value;
                              setActiveResume(updated);
                              handleUpdateResume(updated);
                            }}
                            className="w-full text-xs p-1.5 border rounded"
                          ...
                          <textarea
                            ...
                            onChange={(e) => {
                              const updated = { ...activeResume };
                              updated.data.projects[index].description = e.target.value;
                              setActiveResume(updated);
                              handleUpdateResume(updated);
                            }}
              ...
              <div className="bg-slate-900 px-4 py-3 rounded-2xl flex items-center justify-between text-white shadow-md print:hidden">
                ...
                <div className="flex gap-2">
                  <button
                    id="btn-print-resume"
                    onClick={handlePrint}
                    className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-xs font-bold flex items-center gap-1.5 cursor-pointer text-white"
                  >
                  ...
                  {analysisResult && (
                    <div className="space-y-6" id="analysis-results">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="bg-slate-900 text-white rounded-2xl p-5 text-center flex flex-col items-center justify-center space-y-2">
                          <h4 className="text-xs font-bold uppercase tracking-wider text-indigo-400">Overall Resume Compatibility</h4>
                          ...
                          <p className="text-xs text-slate-400 font-mono">Categorized as: <strong className="text-white text-xs">{analysisResult.category}</strong></p>
                          <p className="text-[10px] text-slate-400 font-mono">Experience: {analysisResult.experienceLevel}</p>
                        </div>

                        {/* ATS Score Circle */}
                        <div className="bg-slate-800 text-white rounded-2xl p-5 text-center flex flex-col items-center justify-center space-y-2">
                          <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-400">ATS Formatting Compatibility</h4>
                          <div className="relative w-24 h-24 flex items-center justify-center">
                            <span className="text-3xl font-black">{analysisResult.atsScore}%</span>
                            <div className="absolute inset-0 border-4 border-emerald-500 rounded-full"></div>
                          </div>
                          <p className="text-xs text-slate-400">Match score out of 100 based on standard section parses.</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100">
                          <h5 className="font-extrabold text-xs text-emerald-800 uppercase tracking-wider flex items-center gap-1.5 mb-2">
                            <CheckCircle className="w-4 h-4" /> Skills Present ({analysisResult.matchingSkills.length})
                          </h5>
                          {analysisResult.matchingSkills.length === 0 ? (
                            <p className="text-xs text-slate-500">No overlapping tech skills parsed.</p>
                          ) : (
                            <div className="flex flex-wrap gap-1.5">
                        ...
                        <div className="bg-rose-50/50 p-4 rounded-xl border border-rose-100">
                          <h5 className="font-extrabold text-xs text-rose-800 uppercase tracking-wider flex items-center gap-1.5 mb-2">
                            <XCircle className="w-4 h-4" /> Skill Gap / Missing ({analysisResult.missingSkills.length})
                          </h5>
                          {analysisResult.missingSkills.length === 0 ? (
                            <p className="text-xs text-emerald-600">Great! Match metrics are fully saturated.</p>
                          ) : (
                            <div className="flex flex-wrap gap-1.5">
                ...
                {activeHistory.map(snapshot => (
                  <div key={snapshot.id} className="bg-white border rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:shadow-md transition">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-mono mt-1">Snapshot captured: {new Date(snapshot.timestamp).toLocaleString()}</p>
                      
                      {snapshot.analysis && (
                        <div className="flex gap-2 items-center mt-2">
                          <span className="text-[10px] font-bold px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded-md">ATS Score: {snapshot.analysis.atsScore}%</span>
                          <span className="text-[10px] font-bold px-2 py-0.5 bg-blue-50 text-blue-700 rounded-md">Predicted Class: {snapshot.analysis.category}</span>
                        </div>
                      )}
                    </div>
